...
Scored day×chain rows: **7** · **n_effective (moving-obs, skill-bearing): 7**
...
|---|---|---|---|---|---|---|
| 45216 | 3 | 6.80 m | 3.98 m | +41% | 3.56 m |  |
| llo1 | 4 | 3.14 m | 3.24 m | -3% | 3.00 m |  |
| **pooled (diagnostic)** | 7 | 4.71 m | 3.56 m | +24% | | moving-obs chains only |

**Read:** the correction beats raw LSOFS pooled (3.56 m vs 4.71 m). Skill is the audit's demotion test (ADR-006): a correction that can't beat raw gets benched.
...
|---|---|---|---|
| +24 h | 10 | 2.03 m | 3.58 m |
| +48 h | 10 | 1.93 m | 3.35 m |
| +72 h | 10 | 2.08 m | 3.60 m |
| +96 h | 10 | 1.54 m | 3.12 m |
| +120 h | 10 | 1.33 m | 2.92 m |
...
n=56 clear scenes · shore-warm **+0.97 °C** (range -11.77…+10.14). GLSEA (1 km) cannot resolve this nearshore warming; too few scenes to fold into the anchor yet (see OVERNIGHT_ITERATION ADR-020).

## Offshore cross-check (LSOFS vs GLERL mooring climatology, ADR-034)

n=36 days · mixed-layer model−clim mean **-0.64 °C**, iso-12 depth model−clim mean **+5.0 m** · 0 DIVERGENT day(s). Independent (observed) check that the model's central-basin thermocline isn't grossly misplaced; generous band (2-yr clim vs 1 day).

## Over-lake wind gate (GFS forecast vs NDBC buoy obs, ADR-032/034)

n=104 buoy-days · GFS speed MAE **2.54 kn**, W-quadrant forecast−obs bias **-0.67 kn** · airport(CYQT)−buoy offset **-1.26 kn** (W-quadrant -1.15). The real over-lake test the wind-model choice deferred; the airport offset is what a separate phase threshold would need to earn — currently within noise, so one Wedderburn bar is used.
