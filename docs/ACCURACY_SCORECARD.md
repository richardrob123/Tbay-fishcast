...
Scored day×chain rows: **3** · **n_effective (moving-obs, skill-bearing): 3**
...
| llo1 | 2 | 2.33 m | 1.22 m | +48% | 0.00 m |  |
| **pooled (diagnostic)** | 3 | 4.01 m | 2.12 m | +47% | | moving-obs chains only |
...
|---|---|---|---|
| +24 h | 8 | 1.68 m | 3.00 m |
| +48 h | 8 | 1.47 m | 2.57 m |
| +72 h | 8 | 1.78 m | 3.01 m |
| +96 h | 8 | 1.23 m | 2.51 m |
| +120 h | 8 | 1.14 m | 2.19 m |
...
n=34 days · mixed-layer model−clim mean **-0.57 °C**, iso-12 depth model−clim mean **+5.1 m** · 0 DIVERGENT day(s). Independent (observed) check that the model's central-basin thermocline isn't grossly misplaced; generous band (2-yr clim vs 1 day).

## Over-lake wind gate (GFS forecast vs NDBC buoy obs, ADR-032/034)

n=100 buoy-days · GFS speed MAE **2.52 kn**, W-quadrant forecast−obs bias **-0.67 kn** · airport(CYQT)−buoy offset **-1.19 kn** (W-quadrant -1.16). The real over-lake test the wind-model choice deferred; the airport offset is what a separate phase threshold would need to earn — currently within noise, so one Wedderburn bar is used.
