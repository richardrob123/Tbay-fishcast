...
n=15 days · mixed-layer model−clim mean **-0.52 °C**, iso-12 depth model−clim mean **+5.4 m** · 0 DIVERGENT day(s). Independent (observed) check that the model's central-basin thermocline isn't grossly misplaced; generous band (2-yr clim vs 1 day).

## Over-lake wind gate (GFS forecast vs NDBC buoy obs, ADR-032/034)

n=42 buoy-days · GFS speed MAE **2.66 kn**, W-quadrant forecast−obs bias **+0.29 kn** · airport(CYQT)−buoy offset **-0.91 kn** (W-quadrant +0.16). The real over-lake test the wind-model choice deferred; the airport offset is what a separate phase threshold would need to earn — currently within noise, so one Wedderburn bar is used.
