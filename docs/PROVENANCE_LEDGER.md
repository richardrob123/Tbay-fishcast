...
| How temp × structure combine | **`intensity = thermal_suitability × structure_strength`** (continuous product) | **METHOD (conjunction, no weights)** | ADR-037: a spot's value = right structure AND right temp = a PRODUCT, which carries ZERO fitted coefficients (rule 7). Glow levels are crossings of this product at the same data-derived structure percentiles, so a break dims through the levels in place as temp goes marginal (never relocates/flips). CAVEAT (unchanged): `bottom_c` is near-planar within a stretch (few LSOFS nodes), so temperature contributes mostly a depth-driven, stretch-scale modulation — which is exactly why it must be a SOFT multiplier, not a crisp per-pixel carve. |
