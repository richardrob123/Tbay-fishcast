...
n=80 buoy-days · GFS speed MAE **2.64 kn**, W-quadrant forecast−obs bias **-0.51 kn** · airport(CYQT)−buoy offset **-1.25 kn** (W-quadrant -1.04). The real over-lake test the wind-model choice deferred; the airport offset is what a separate phase threshold would need to earn — currently within noise, so one Wedderburn bar is used.
